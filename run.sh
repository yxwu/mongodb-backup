...
BACKUP_CMD="mongodump --archive=/backup/"'${BACKUP_NAME}'" --host ${MONGODB_HOST} --port ${MONGODB_PORT} --gzip ${USER_STR}${PASS_STR}${DB_STR} ${EXTRA_OPTS}"
...
BACKUP_NAME=\$(date +\%Y.\%m.\%d.\%H\%M\%S).gz
...
if mongorestore --host ${MONGODB_HOST} --port ${MONGODB_PORT} --gzip --drop ${USER_STR}${PASS_STR} ${EXTRA_OPTS_RESTORE} --archive=/backup/\$1; then
